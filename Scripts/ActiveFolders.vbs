<FILE file_path="/home/workdir/attachments/ActiveFolders.vbs" size="5273 bytes">' ActiveFolders.vbs
...
Sub PopulateActiveFolders()
    ...
    If doUpdate Then
        ...
        Dim activeSelect
        Set activeSelect = Document.GetElementById(Id.List.ActiveFolders)
...
Sub ActiveFolders_OnChange()
    Dim selectedPath
    selectedPath = Document.GetElementById(Id.List.ActiveFolders).Value
    If selectedPath <> "" And selectedPath <> "Select Folder" Then
        Populatelist selectedPath
        ' Save to registry if desired, similar to Getfolder_Onclick
        On Error Resume Next
        Oshell.RegWrite "HKEY_CURRENT_USER\Software\BatchPlotter\LastFolder", selectedPath, "REG_SZ"
        On Error GoTo 0
        ' Reset to default "Select Folder" option
        Document.GetElementById(Id.List.ActiveFolders).Value = ""
    End If
End Sub
